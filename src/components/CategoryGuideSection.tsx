    ...
    <section className="mb-10 space-y-8">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {guide.links.map((link) => (
              <a
                ...
                className="inline-flex items-center gap-1 rounded-md border border-white/10 bg-white/[0.03] px-3 py-1.5 text-sm text-neutral-200 transition hover:border-indigo-400/40 hover:bg-white/[0.06]"
              >
                {link.label} ↗
              </a>
            ))}
      ...
      <div>
        ...
        <p className="mb-4 text-xs text-neutral-500">
          ป้าย <span className="text-emerald-400">✅ ทดสอบแล้ว</span> แปลว่ามีคนรัน goodPrompt
          จริงและแนบ output เป็นหลักฐานไว้ (ดูได้จากช่อง &ldquo;หลักฐาน&rdquo; ท้ายแต่ละตัวอย่าง)
          — แต่ถ้าเป็นการ self-test เพียงครั้งเดียว ยังไม่ผ่านการรีวิวจากพนักงานจริงในหลายๆ เคส
          ก็ควรทดสอบซ้ำกับงานจริงขององค์กรก่อนแนะนำใช้งานเป็นวงกว้าง ส่วนป้าย{" "}
          <span className="text-amber-400">⚠️ ยังไม่ทดสอบผลจริง</span>{" "}
          คือตัวอย่างที่เขียนจากหลักการทั่วไปเท่านั้น ยังไม่มีใครลองรันจริง
        </p>
        <div className="space-y-6">
          {guide.promptTemplates.map((template, i) => (
            <div key={i} className="rounded-lg border border-white/10 p-5">
                ...
                {template.sourceUrl ? (
                  <>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="underline decoration-neutral-600 underline-offset-2 hover:text-indigo-400"
                    >
                      (แหล่งอ้างอิง ↗)
                    </a>
                  </>
              ...
              {template.sampleOutput ? (
                <details className="rounded border border-white/10 bg-black/40">
                  ...
                  <p className="whitespace-pre-wrap border-t border-white/10 px-3 py-2 text-xs leading-relaxed text-neutral-400">
                    {template.sampleOutput}
                  </p>
                </details>
              ) : null}
